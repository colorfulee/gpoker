...
//			}
		 ...
		 * 创建movieclip
		 * */
		public function createMovieClip(name:String,data:Array=null, textureAtlas:TextureAtlas = null):SwfMovieClip{
			...
			for(var objName:String in objectCountData){
				...
				for (var i:int = 0; i < count; i++) {
					...
					displayObjectArray.push(fun(objName,null,textureAtlas));
				}
		...
		/**
		 * 创建图片
		 * preTexture 预设的素材集合，避免特效素材重名,导致被替换
		 * */
		public function createImage(name:String,data:Array=null,preTexture:TextureAtlas = null):SwfImage{
			var texture:Texture = preTexture == null ? _assets.getTexture(name) : preTexture.getTexture(name);
			if(texture == null)
