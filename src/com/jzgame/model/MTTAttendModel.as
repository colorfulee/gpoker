package com.jzgame.model
{
	...
	public class MTTAttendModel
	{
		...
		*/
		...
		//mtt match id
		private var _MTTMatchId:String;
		//spe
		...
		//multi_table玩法
		private var _tournamentMT:Array = [];
		...
		public function MTTAttendModel()
		{
		}
		...
		public function get MTTMatchId():String
		{
			return _MTTMatchId;
		}

		public function set MTTMatchId(value:String):void
		{
			_MTTMatchId = value;
		}
		...
		{
			var tableInfo:MTTTableListInfo = getMTTTableInfoByID(MTTMatchId);
			...
			if(left<_remindTime && left > 59)
			{
				...
				Tracer.info(MTTMatchId+",MTTRemindCommand remind me later by:"+left+" seconds");
		 ...
		 * 
		 */		
		public function updateMTTList(object:Object):void
		{
			_tournamentMT.splice(0,_tournamentMT.length);
			updateSpeMTTRoomListInfo(object,_tournamentMT);
			
			_tournamentMT.sortOn("start_time",Array.NUMERIC);
		}
		/**
		 * 根据match id 获取信息 
		 * @param match_id
		 * 
		 */		
		public function getMttInfoByMatchId(match_id:String):MTTTableListInfo
		{
			for(var index:String in _tournamentMT)
			{
				if(_tournamentMT[index].match_id == match_id)
				{
					return _tournamentMT[index];
				}
			}
		...
		private function updateSpeMTTRoomListInfo(value:Object,lib:Array):void
		{
			...
			for(var index:String in ringNew)
			{
				...
				tableListInfo.requirement = ringNew[index]["cost_chip"];
		...
		private function checkCounting(index:String):void
		{
			...
			if(attendList.hasOwnProperty(index))
			{
				var tableListInfo:MTTTableListInfo = getMTTTableInfoByID(index);
				//判断是否存在，避免异步出问题
				if(tableListInfo)
				{
					var startTime:Number = 0;
					...
					if(vo.status == 1)
					{
						...
						var left:Number = startTime - TimerManager.getCurrentSysTime();
						Tracer.info(index + "剩余时间为:"+left + ":开赛时间:"+startTime+":服务器时间:"+TimerManager.getCurrentSysTime());
						//如果存在剩余时间
						...
						{
							Tracer.info(index + "剩余时间为:"+left+":我的状态为:"+vo.status);
						}
					}else
					{
						Tracer.info(index +":我的状态为:"+vo.status);
					}
				}
			}else
			{
				Tracer.info(index + "当前房间我没报名:");
			}
		}
		
		private function updateRemind(vo:MTTAttendVO):void
		{
			var tableInfo:MTTTableListInfo = getMTTTableInfoByID(vo.matchID);
			...
			if(left<_remindTime)
			{
				if(tableInfo.status == 2)
				{
					if(gameModel.inTable)
					{
						...
						{
							WindowFactory.addPopUpWindow(WindowFactory.MTT_IN_TABLE_REMIND_WINDOW,null,tableInfo.rewards,TimerManager.getTimerFormat(tableInfo.start_time,"hh:ii"),tableInfo.img);
						}
					}else
					{
						WindowFactory.addPopUpWindow(WindowFactory.MTT_REMIND_WINDOW,null,tableInfo.rewards,left,TimerManager.getTimerFormat(tableInfo.start_time,"hh:ii"));
					}
					
					MTTMatchId = vo.matchID;
				}else
				{
					Tracer.info(vo.matchID+",MTTRemindCommand less 5 but table status != 2 等20秒再刷一次");
					setTimeout(timerHandler,(20)* 1000);
				}
			}else
			{
				Tracer.info(vo.matchID+",MTTRemindCommand 看看剩余时间:"+left+":"+(left -  _remindTime)* 1000);
		...
		{
			Tracer.info("发送请求同步状态");
			//时间到了同步下状态
			HttpSendProxy.sendMTTList();
		}
		 ...
		 * 
		 */		
		public function getMTTTableInfoByID(matchId:String):MTTTableListInfo
		{
			for each(var vo:MTTTableListInfo in _tournamentMT)
			{
