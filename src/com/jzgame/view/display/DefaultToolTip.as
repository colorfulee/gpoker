package com.jzgame.view.display
{
	...
	public class DefaultToolTip extends SLUIComponent implements IToolTip
	{
		/*auther     :jim
...
//		protected var _label:SLLabel = new SLLabel;
		...
		public function DefaultToolTip(tip:String)
		{
			super();
...
//			_arrow.scaleY = -1;
		...
		{
			_rect = rect;
			//由于窗口放大，导致坐标失真，需要重新计算
			this.x = Math.floor((rect.x + rect.width * 0.5) / ReleaseUtil.SCALEX);
			this.y = Math.floor((rect.y - _arrow.y - _arrow.height) / ReleaseUtil.SCALEX);
			if(this.x < 0)
			{
