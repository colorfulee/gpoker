package com.jzgame.view.windows.friends
{
	...
	public class FriendsListItem extends DefaultListItemRenderer
	{
		...
		override protected function initialize():void
		{
			...
			_join.addEventListener(Event.TRIGGERED, joinHandler);
			_invite.addEventListener(Event.TRIGGERED, inviteHandler);
		}
...
//					_invite.visible = true;
					...
					_join.visible = true;
					_time.text = 'in table ' + fvo.tableId;
					break;
		...
		{
			if(_data && _data.tableId)
				AssetsCenter.eventDispatcher.dispatchEvent(new HandleJoinTableEvent(_data.tableId.toString()));
		}
		/**
		 * 邀请 
		 * @param e
		 * 
		 */		
		private function inviteHandler(e:Event):void
		{
			NetSendProxy.inviteFriendGame(int(GameFriendVO(_data).id));
			
			WindowFactory.addPopUpWindow(WindowFactory.FADE_TIP_WINDOW, null, LangManager.getText('500261'));
		}
		...
		override public function dispose():void
		{
			...
			_face.removeEventListener(TouchEvent.TOUCH, showInfoHandler);
			_join.removeEventListener(Event.TRIGGERED, joinHandler);
		}
	}
}